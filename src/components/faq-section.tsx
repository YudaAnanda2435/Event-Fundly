  ...
  return (
    <section id="faq" className="w-full py-12 md:py-24 lg:py-32 bg-black">
      <div className="container px-4 md:px-6">
        {/* FAQ Header */}
        <div className="flex flex-col items-start px-24 space-y-4 text-center">
          <h2 className="text-3xl font-fontJakarta font-semi-bold tracking-tighter sm:text-4xl md:text-5xl text-white">
            Frequently Asked Questions
          </h2>
          <p className="max-w-[700px] font-fontJakarta text-neutral md:text-xl">
        ...
        <div className="mx-auto grid max-w-5xl grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mt-12">
          {faqs.map((faq, index) => (
            <div key={index} className="space-y-2">
              <h3 className="text-lg font-base  font-fontJakarta text-white">
                {faq.question}
              </h3>
              <p className="text-secondary font-fontJakarta">{faq.answer}</p>
            </div>
          ))}
        </div>

        {/* Contact section */}
        <div className="mt-24 items-start px-24 space-y-4">
